...
@HiltViewModel
class EAUpComingMoviesViewModel @Inject constructor(
    ...
    fun getUpComingMovies(){
        viewModelScope.launch {
            moviesRepository.getUpComingMovies().collectLatest {
                when(it.status){
    ...
    private fun updateMovies(listMovies: List<EAMovie>?){
        listMovies?.let {
            ...
            popularList.forEach{
                it.type = EAConstants.TYPE_UP_COMING
            }
    ...
    private fun getLocalMovies(){
        viewModelScope.launch(Dispatchers.IO){
            moviesRepository.getLocalUpComingMovies().collectLatest { localMovies ->
                if (localMovies == null){
